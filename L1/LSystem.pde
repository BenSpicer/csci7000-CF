 ...
 // Production rules
  ...
  public void iterate() {
    ...
    for (int i = 0; i < current.length(); i += 1) {
      String new_str = this.rules.get(current.charAt(i));
      if (new_str != null) currentIterationBuffer.append(new_str);
      else currentIterationBuffer.append(current.charAt(i));
    }
